(() => {
    ...
    try {
      ...
      permissionText.textContent =
        err && err.name === "NotAllowedError"
          ? "Camera access was denied. Enable it in your browser or app settings."
  ...
  // ---------- Flash (screen-flash effect, not hardware flash) ----------
  ...
  function capturePhoto() {
    ...
    // Mirror the image if using the front camera, so it matches what was on screen
    if (facingMode === "user") {
  ...
  function saveShots() {
    try {
      ...
      // Storage full or unavailable — capture still works, just won't persist.
    }
  }
  ...
  function renderCounter() {
    counterEl.textContent = String(shots.length).padStart(3, "0");
  }
  ...
  viewerDelete.addEventListener("click", () => {
    ...
    if (shots.length === 0) {
      ...
      currentViewerIndex = shots.length - 1;
      renderViewer();
    } else {
      renderViewer();
    }
